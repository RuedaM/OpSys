...
int RRalt(struct Process* allProcesses, int n, int t_cs, int t_slice, int fd, ssize_t bytesWritten, char toWrite[200]){
    int time = 0;
    #if DEBUG_MODE
    int timeAdd = 0;
    #endif
    ...
    while(1){
        ...
        if (preemptFlag){ // If preempt has been triggered, treat the current CPU process as if it had been completed
            if(csLeft==0 && cpuProc->state==4){ // If done cs'ing...
                ...
                // Shift elements from position 2 onwards to the right by 1
                for (int i=priorityQueueLen ; i>=1 ; i--) {priorityQueue[i] = priorityQueue[i-1];}
                priorityQueue[1] = cpuPop(cpuProc); // Remove curr-proc, add second position in the queue
                priorityQueue[1]->state = 1; //state=IN-QUEUE
                priorityQueueLen += 1;
                preemptFlag = 0; //Reset
