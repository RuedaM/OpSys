...
int SRTnoTau(struct Process* allProcesses, int n, int t_cs, double alpha, int fd, ssize_t bytesWritten, char toWrite[200]){
    ...
    while(1){
        ...
        //======================================================================================================================
        #if DEBUG_MODE
        printf("~~ Checking if processes can be added from queue or I/O (preemption) to CPU or Pre-CPU stage...\n");
        #endif
        if (priorityQueueLen!=0 && preemptFlag && csPreemptLeft==0){
            #if DEBUG_MODE
            printf("~~ Preemption should be ocurring...\n");
            #endif
            preemptFlag = 0;
            csPreemptLeft = t_cs;
            ...
            for (int i=0 ; i<priorityQueueLen ; i++) {if (priorityQueue[i]->preempting) {preemptProc = priorityQueue[i];}}
            ...
            if (time<10000){
                ...
                else {printf(" %dms burst", cpuProc->cpuBurstCurr);}
                priority_queue_status(priorityQueue, priorityQueueLen);
            }
            cpuProc->preempting = 0;
        } 

        else if (priorityQueueLen!=0 && !cpuIsRunning){ // If queue isn't empty and nothing is in the CPU...
        ...
        //======================================================================================================================
        #if DEBUG_MODE
        printf("~~ Checking if process can be added from I/O back to queue OR preempt the CPU process...\n");
        #endif
        ...
        if (preemptFlag && csPreemptLeft!=0){ // If cs for preempting process from I/O is smallest, adv by that
            if (csPreemptLeft<advance){
                advance = csPreemptLeft;
                #if DEBUG_MODE
                printf("~~ Found new advancing time: cs time for preemption of %d\n", advance);
                #endif
            }
        }
        ...
        if (preemptFlag && csPreemptLeft!=0){ // Im preemption will occur, decrease preemption cs time
            csPreemptLeft -= advance;
            #if DEBUG_MODE
            printf("~~ Preemption ocurring: decreasing preemption cs time\n");
            #endif
        }
        ...
        printf("~~ queueLen = %d\n", priorityQueueLen);
        printf("~~ ioLen = %d\n", ioLen);
        if (preemptFlag) {printf("~~ csPreemptLeft = %d\n", csPreemptLeft);}
        printf("~~ ============================================================================\n");
