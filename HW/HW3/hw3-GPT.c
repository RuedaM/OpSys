...
int solve(int argc, char** argv){
    ...
    // moving(&firstMove);
    move_t*firstMove = calloc(1,sizeof(move_t));
    *firstMove = (move_t){
        .board = board,
        .m = m, .n = n,
        .r = r, .c = c,
...
void* moving(void* arguments){
    ...
    *(*(move->board+move->rNew)+move->cNew) = 'X';
    ...
    if (move->r+2<move->m && move->c+1<move->n && *(*(move->board+move->r+2)+move->c+1)!='X') { // Up-Right
    ...
    if (move->r+1<move->m && move->c+2<move->n && *(*(move->board+move->r+1)+move->c+2)!='X'){ // Right-Up
        *(possCoords+pCCount) = calloc(2, sizeof(int));
    ...
    if (move->r-1>=0 && move->c+2<move->n && *(*(move->board+move->r-1)+move->c+2)!='X'){ // Right-Down
        *(possCoords+pCCount) = calloc(2, sizeof(int));
        *(*(possCoords+pCCount)+0) = move->r-1;
        *(*(possCoords+pCCount)+1) = move->c+2;
        pCCount++;
    ...
    if (move->r-2>=0 && move->c+1<move->n && *(*(move->board+move->r-2)+move->c+1)!='X'){ // Down-Right
        *(possCoords+pCCount) = calloc(2, sizeof(int));
        *(*(possCoords+pCCount)+0) = move->r-2;
        *(*(possCoords+pCCount)+1) = move->c+1;
        pCCount++;
    ...
    if (move->r-2>=0 && move->c-1>=0 && *(*(move->board+move->r-2)+move->c-1)!='X'){ // Down-Left
        *(possCoords+pCCount) = calloc(2, sizeof(int));
        *(*(possCoords+pCCount)+0) = move->r-2;
        *(*(possCoords+pCCount)+1) = move->c-1;
        pCCount++;
    ...
    if (move->r-1>=0 && move->c-2>=0 && *(*(move->board+move->r-1)+move->c-2)!='X'){ // Left-Down
        *(possCoords+pCCount) = calloc(2, sizeof(int));
        *(*(possCoords+pCCount)+0) = move->r-1;
        *(*(possCoords+pCCount)+1) = move->c-2;
        pCCount++;
    ...
    if (move->r+1<move->m && move->c-2>=0 && *(*(move->board+move->r+1)+move->c-2)!='X'){ // Left-Up
        *(possCoords+pCCount) = calloc(2, sizeof(int));
        *(*(possCoords+pCCount)+0) = move->r+1;
        *(*(possCoords+pCCount)+1) = move->c-2;
        pCCount++;
    ...
    if (move->r+2<move->m && move->c-1>=0 && *(*(move->board+move->r+2)+move->c-1)!='X'){ // Up-Left
        *(possCoords+pCCount) = calloc(2, sizeof(int));
        *(*(possCoords+pCCount)+0) = move->r+2;
        *(*(possCoords+pCCount)+1) = move->c-1;
        pCCount++;
    ...
    // Free possible move coordinates
    for (int i=0 ; i<pCCount ; i++) {free(*(possCoords+i));}
    free(possCoords);

    // Free the board and move struct if heap-allocated
    if (move->threadID!=move->mainThreadID) { // Avoid freeing the initial stack-allocated move
    ...
    int* ret = calloc(1, sizeof(int)); // Be sure to free outside of function call!
    *ret = currTID;
    return ret;
